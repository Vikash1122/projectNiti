                ...
                $.each(json, function(index, value) {
                    ...
                    if(value.index != ''){
                        ...
                        if(value.profile_pic == null || value.profile_pic == ''){
                            var emp_image = '<img src="{{ asset("public/img/defaultIcon.png") }}" class="">';                    
                        }else{
                            ...
                            '<div class="row">'+
                                ...
                                if(value.group != ''){
                                    var teamleader_image='';
                                                ...
                                                if(value.group.teamleader_image == null || value.group.teamleader_image == ''){
                                                    var teamleader_image = '<img src="{{ asset("public/img/profiles/avatar_small.jpg") }}" alt="" data-src="{{ asset("public/img/profiles/avatar_small.jpg") }}" >';
                                    ...
                                    var teamleader_image='';
                                        ...
                                        if(value.empIMG == null || value.empIMG == ''){
                                            var teamleader_image = '<img src="{{ asset("public/img/defaultIcon.png") }}" class="">';
...
<script>
 $("#service_Ids").on('change',function(){
    ...
    if(serviceID){
        $.ajax({
           type:"GET",
           url:"{{url('admin/inventry/products/getajaxProdata')}}?service_id="+serviceID,
           success:function(res){   
               // alert(res);          
            if(res){
...
$('#product_Ids').on('change',function(){
   ...
   // alert(productID);
    if(productID){
        $.ajax({
           type:"GET",
           url:"{{url('admin/inventry/products/ajaxproductdata')}}?product_id="+productID,
           success:function(res){               
            if(res){
...
$('#sService_type').on('change',function(){
    ...
    if(serviceID){
        $.ajax({
           type:"GET",
           url:"{{ url('admin/categories/getajaxSubServicedata') }}?service_id="+serviceID,
           success:function(res){               
            if(res){
                var json = $.parseJSON(res);
...
$('#subService_type').on('change',function(){
    ...
    if(subserviceID){
        $.ajax({
           type:"GET",
           url:"{{url('admin/inventry/products/ajaxCategorydata')}}?subservice_id="+subserviceID,
           success:function(res){               
            if(res){
                var json = $.parseJSON(res);
                $("#category_id").empty();
