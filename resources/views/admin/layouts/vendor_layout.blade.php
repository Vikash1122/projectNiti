...
<link href="{{ asset('public/developer/css/form_wizard.css') }}" rel="stylesheet" type="text/css" />
...
@yield('js')
<script src="{{ asset('public/developer/js/form_wizard.js') }}" type="text/javascript"></script>
<script>
...
function selectService(id){
    ...
    if($("#"+id).val()){
        $.ajax({
           type:"GET",
           url:"{{ url('admin/categories/getajaxSubServicedata') }}?service_id="+$("#"+id).val(),
           success:function(res){     
            if(res){
                var json = $.parseJSON(res);
                $("#subService_type"+ID).html('');
...
function selectCategory(id){
    var ID=id.substring(15);
     if($("#"+id).val()){
        $.ajax({
           type:"GET",
           url:"{{url('admin/inventry/products/ajaxCategorydata')}}?subservice_id="+$("#"+id).val(),
           success:function(res){   
            if(res){
                var json = $.parseJSON(res);
                $("#category_id"+ID).html('');
