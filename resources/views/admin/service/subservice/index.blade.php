...
@section('content')
    ...
    <div class="content">
                ...
                <div class="col-md-12">
                    ...
                    <div class="serchBarDiv">
                        <div class="searchContainer">
                            <input class="searchBox" type="search" id="sub_ServiceSearch" data-id="{{ $id }}" name="search" placeholder="Search">
        ...
        @if(isset($subservices[0]) && !empty($subservices[0]))
                                        ...
                                        <table class="table table-condensed">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <td>Unit Price</td>
                                                    <td>Status</td>
                                                    @if(hasPermissionThroughRole('edit_sub_services') || hasPermissionThroughRole('delete_sub_services'))
                                            ...
                                            <tbody id="searchData1">
                                                ...
                                                <tr>
                                                    <td>{{ $sub->services->service_code }}</td>
                                                    <td>{{ $sub->sub_service_name}}</td>
                                                    <td>{{ $sub->unit_variable}}</td>
                                                    <td>{{ $sub->unit_duration}} </td>
                                                    <td class="viewBtnUnitPrice">
                                                        <img class="viewIconImg" id="{{ $sub->id }}" onclick="viewPrice({{ $sub->id }})" src="../../../public/img/visibility.png" width="42" />
                                                        <span onclick="hidePrice({{ $sub->id }})"  id="abd{{ $sub->id }}" class="hidden showPrice">{{ $sub->unit_price}} AED</span>
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($sub->status == 1)
                                                            <label class="switch">
                                                                <input name="status" type="checkbox" onclick="statusActive(<?php echo $sub->id; ?>)" value="0" id="enable_{{$sub->id}}" checked>
                                                                <span class="slider round"></span>
                                                            </label>
                                                        @else
                                                            <label class="switch">
                                                                <input name="status" type="checkbox" onclick="statusActive(<?php echo $sub->id; ?>)" value="1" id="enable_{{$sub->id}}">
                                                                <span class="slider round"></span>
                                                            </label>
                                                        @endif
                                                        </div>
                                                    ...
                                                    @if(hasPermissionThroughRole('edit_sub_services'))
                                                    <button type="button" title="Edit Sub Services" class="edit_btn">
                                                        <a href="{{ url('admin/services/'.$sub->service_id.'/editSubSErvice/'.$sub->id) }}">
                                                            <i class="fa fa-pencil"></i>
                                                        </a>
                                                    </button>
                                                    @endif
                                                    @if(hasPermissionThroughRole('delete_sub_services'))
                                                    <button type="button" class="delete_btn" title="Delete Sub Service">
                                                        <a href="{{ url('admin/services/destroySubService',$sub->id)}}" data-id="{{ $sub->id }}" onclick="return confirmation();">
                    ...
                    $.each(json,function(index, value){
                        ...
                        htmlslot=
                            '<tr>'+
                                ...
                                '<td class="viewBtnUnitPrice">'+
                                    '<img class="viewIconImg" id="'+value.id+'" onclick="viewPrice('+value.id+')" src="{{ asset("public/img/visibility.png") }}" width="42" />'+
