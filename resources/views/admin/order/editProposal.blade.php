...
@section('content')
    ...
    <div class="content">
        ...
        @if(isset($jobcard[0]) && !empty($jobcard[0]))
            <form action="{{ url('admin/orders/updateProposal/'.$id) }}" method="post">
                {{ csrf_field() }}
                                    ...
                                    <table class="table">
                                        <thead>
                                            <tr style="background: #1b1e24;">
                                                <th>#</th>
                                                <th>Service Name</th>
                                                @if(isset($jobcard[0]->text_content) && !empty($jobcard[0]->text_content))
                                                    <th>Description</th>
                                                @endif
                ...
                @if($jobcard[0]->text_content == '' || $jobcard[0]->text_content == NULL)
                    <div class="row">
                        <div class="col-md-12">
                            <div class="content-box-main">
                                                                ...
                                                                <div class="col-md-12">
                                                                    <table class="table table-hover">
                                                                        ...
                                                                        @if(isset($card['serviceData'][0]) && !empty($card['serviceData'][0]))
                                                                            ...
                                                                            <tr>
                                                                            <td>
                                                                                <input type="hidden" name="sub_service_id_{{$card->service_id}}[]" value="{{$sub->sub_service_id}}">
                                                                                {{ $sub->sub_service_name }}
                                                                                </td>
                                                                                <td>
                                                                                <input type="hidden" name="product_id_{{$card->service_id}}[]" value="{{$card->product_id}}">
                                                                                {{ $sub->product_name }}
                                                                                </td>
                                                                                <td>{{ $sub->qty }}</td>
                                                                                <td>{{ $sub->unit_variable }}</td>
                                                                                <td class="text-center">
                                                                                <input type="text" name="sub_service_price_{{$card->service_id}}[]" id="" class="serTextInput11" value="{{ $sub->sub_price }}" readonly />
                                                                                </td>
                                                                            </tr>
                                                                            @endforeach
                                                                        @endif
                                                                        ...
                                                                        <tfoot>
                                                                            <tr>
                                                                                ...
                                                                                {{ $card['sub_total_price'] }}
                                                                                    <!-- <input type="" name="" id="" class="serTextInput11" value="{{ $card['sub_total_price'] }}" disabled="disabled" /> -->
                                                                                </th>
                                                                            </tr>
                                                                        </tfoot>
                                                                    </table>
                                                ...
                                                <div class="content">
                                                    <div class="contentBox">
                                                        <div class="order_history_box">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-12">
                                                                @foreach($jobcard as $cd)
                                                                    <p class="topPadding15">{{ $cd['service_name'] }} 
                                                                        <span class="pull-right">
                                                                        <input type="text" name="service_price_{{$cd['service_id']}}[]" id="" class="serTextInput" value="{{ $cd['sub_total_price'] }}" readonly /></span>
                                                                    </p>
                                                                @endforeach
                                                                <?php 
                                                                    $sum = 0;
                                                                ...
                                                                ?>
                                                                    ...
                                                                    <p class="amtPay">Total Price <span class="pull-right" id="totalPrice">{{ $sum }}</span></p>
                                                                    ...
                                                                    <input type="text" name="total_price" id="grandTotal" class="serTextInput" value="{{ $sum }}" readonly /></span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                ...
                @endif
                    <div class="row">
                        <div class="col-md-offset-8 col-md-4">
                            <div class="chkOutBox"><button type="submit" class="btn btn-block btn-primary">Update Proposal</button></div>
                        </div>
                    </div>
                </div>
            </form>
  ...
  <script>
    ...
    function jobIsReject(id){
        $.ajax({
            url : "{{ route('isReject.get') }}?job_id="+id,
            type: "GET",
            async : false ,
            success : function(data){
                ...
                if(data > 0){
                    ...
                    setTimeout(function(){ 
                        location.reload();
                        window.location.href = "{{ url('admin/orders/viewProposal') }}/"+id;
                    }, 2000);
